...
However, when dealing with pure qubit states, we in fact only need two coordinates instead of three.  Instead of using xyz coordinates, it is more convenient to use \textit{spherical coordinates} to locate a point on the surface of the Bloch sphere.  By doing so, we can relate between our statevector of two complex numbers and the three xyz coordinates.
...
\begin{figure}[H]
	\ctikzfig{longitude}
	\caption{Perhaps the most familiar example is the Equator, the circle containing all points on the Earth's surface that have a polar angle of $90\deg$.
...
From spherical coordinates, it is straightforward to translate to xyz coordinates:
\begin{equation}
	\begin{bmatrix}
		x\\
		y\\
...
\begin{equation}
	...
	\end{bmatrix}
\end{equation}
For any pure qubit state, we can apply the I gate to it by multiplying its statevector by the identity matrix.
...
\begin{example}[Applying the X gate]
	...
	\begin{equation}
		...
		\end{bmatrix} = -\kx-
	\end{equation}
	\textnormal{The resulting state is $\kx-$, with a factor of $-1$.  If you have one qubit in the $\kx-$ state and another qubit in the $-\kx-$ state, it is impossible to distinguish between them.  For some intuition as to why, recall that how we gather intel on an unknown quantum state is by testing for which one it becomes of the states it can be measured to --- whatever the original state was, after quantum measurement the state is now the observed state.  If we were to test for the $\kx-$ state, the probability the test succeeds would be}
	\begin{equation}
		\text{Pr}(\kx-|\psi) = \abs{\langle-|\psi\rangle}^2
	\end{equation}
	\textnormal{for arbitrary pure qubit state $\psi$.
	This is the same as testing for the $-\kx-$ state.  Indeed, these two tests (well, they are the same test) have the same probability of succeeding:}
	\begin{align}
		\text{Pr}(-\kx-|\psi) &= \abs{-\langle-|\psi\rangle}^2\\
		&= \left(\abs{-}\abs{\langle-|\psi\rangle}\right)^2\\
		&= \abs{\langle-|\psi\rangle}^2\\
		&= \text{Pr}(\kx-|\psi)
	\end{align}
	\textnormal{This factor of $-1$ is a \textbf{global phase}.  In general, two quantum states are the same up to a global phase, if one of them is a factor (which is not $1$) of the other.  For probabilities to be sensible, this factor must have absolute value 1, i.e. be of the form $e^{i \alpha}$ for some real number $\alpha$ (in the range $[0, 2\pi)$).  In this case, $\alpha = \pi$ because $-1 = e^{i \pi}$.  Thus, to state more accurately, each pure qubit state up to a global phase corresponds to a point on the surface of the Bloch sphere.}

	\textnormal{Now with the knowledge that the X gate does not change either of the X-basis states, it makes more sense why they are called the X-basis states.  The X-basis statevectors are \textit{eigenvectors} of the X gate matrix.  We will later learn that like the Z-basis states, their statevectors form an (orthonormal) basis for any pure qubit statevector.}
\end{example}
